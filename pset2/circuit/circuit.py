...
class PriorityQueue :
    ...
    """@index 1-indexed index
    @returns : 1 indexed location of left child if there is otherwise None"""
    def getLeft(self, index):
        if index > len(self) or (2 * index )> len(self):
            return None

        return 2 * index

    def getRight(self, index):
        if index > len(self) or (2 * index + 1 )> len(self):
            return None
    ...
    def min_Heapify(self, index):
        ...
        if(left <= len(self) and self.getValue(index) > self.getValue(left)):
            smallest = left

        if right != None:
            if(right <= len(self) and self.getValue(smallest) > self.getValue(right)):
                smallest = right

        if smallest != index:
            self.queue[smallest - 1], self.queue[index - 1] = self.queue[index -1], self.queue[smallest - 1]
        else:
            return
    ...
    def append(self, item):
        if item == None:
            raise ValueError("can not insert None")
        self.queue.append(item)
        # self.buildMinHeap()
        self.shiftUp(len(self))
    ...
    def shiftUp(self, index):
        ...
        if self.queue[index - 1] < self.queue[parent - 1] :
            ...
            self.queue[index - 1], self.queue[parent - 1] = self.queue[parent - 1], self.queue[index - 1]
            # print(f"self.array[index - 1] : {self.array[index - 1]} self.array[parent - 1] : {self.array[parent - 1] }")
    ...
    def min(self):
        if len(self) == 0:
            return None
        self._find_min()
        return self.getValue(self.min_index)
    ...
    def pop(self):
        ...
        self._find_min()
        self.queue[0], self.queue[len(self) - 1] = self.queue[len(self) - 1], self.queue[0]
        item = self.queue.pop()
        self.min_Heapify(1)
        return item  
