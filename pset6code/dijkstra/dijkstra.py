...
class NodeDistancePair(object):
    ...
    #     return (self.distance == other.distance and id(self.node) == id(other.node) and 
    #             self.node.latitude == other.node.latitude and self.node.longitude == other.node.longitude 
    #             and self.node.state == other.node.state)

    # def __hash__(self):
    #     return hash(tuple([self.distance, self.node.latitude, self.node.longitude, self.node.state]))
...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        # # print(q.extract_min())
        # q.insert(7)
        # print(f"heap before decrease_key{q.heap}")
        # print(f"index for value 8 : {q.key_index[8]}")
        # q.insert(1)
        # q._min_heapify(len(q.heap))
        ...
        while Q:
            ...
            for v in u.node.adj:
                if v.key == None:
                    v.key = NodeDistancePair(v, (u.distance + weight(u.node , v)))
                    v.parent = u.node
                    Q.insert(v.key)
                elif v.key.distance > (u.distance + weight(u.node , v)):
                    v.key.distance = u.distance + weight(u.node , v)
                    v.parent = u.node
